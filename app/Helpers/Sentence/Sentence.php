...
class Sentence
{
     ...
     * @return array
     */
    private function mbSplit($pattern, $string, $limit = -1, $flags = 0)
    {
        ...
        foreach ($lengths as $length) {
            ...
            if ($limit > 0 && ! $is_delimiter && ($length[0] || ~$flags & PREG_SPLIT_NO_EMPTY) && ++$count > $limit) {
                if ($length[0] > 0 || ~$flags & PREG_SPLIT_NO_EMPTY) {
                    $parts[] = ($flags & PREG_SPLIT_OFFSET_CAPTURE) !== 0 ? [mb_strcut($string, $position), $position] : mb_strcut($string, $position);
                }
                break;
            } elseif ((! $is_delimiter || ($flags & PREG_SPLIT_DELIM_CAPTURE && $is_captured)) && ($length[0] || ~$flags & PREG_SPLIT_NO_EMPTY)) {
                $parts[] = ($flags & PREG_SPLIT_OFFSET_CAPTURE) !== 0 ? [mb_strcut($string, $position, $length[0]), $position] : mb_strcut($string, $position, $length[0]);
            }
     ...
     * @return string[]
     */
    private function linebreakSplit($text)
    {
        $lines = [];
        $line = '';

        foreach ($this->mbSplit('([\r\n]+)', $text, -1, PREG_SPLIT_DELIM_CAPTURE) as $part) {
            $line .= $part;
            if ($this->mbTrim($part) === '') {
                $lines[] = $line;
                $line = '';
     ...
     * @return string
     */
    private function cleanUnicode($string)
    {
    ...
    private function abbreviationMerge($fragments)
    {
        ...
        foreach ($fragments as $fragment) {
            ...
            $words = mb_split('\s+', $this->mbTrim($fragment));
            ...
            $last_is_abbreviation = str_ends_with(trim($fragment), '.');
    ...
    private function closeQuotesMerge($statements)
    {
        ...
        foreach ($statements as $statement) {
            ...
            if (trim($statement) === '"' || trim($statement) === "'" ||
                    (
                        (str_starts_with($statement, '"') || str_starts_with($statement, "'")) && substr($statement, 1, 1) === ' ' && ctype_lower(substr($statement, 2, 1))
                    )
            ) {
    ...
    private function sentenceMerge($shorts)
    {
        ...
        foreach ($shorts as $short) {
            $word_count = count(mb_split('\s+', $this->mbTrim($short)));
    ...
    public function split($text, $flags = 0)
    {
        ...
        // Split
        foreach ($this->linebreakSplit($text) as $line) {
            if ($this->mbTrim($line) !== '') {
        ...
        // Post process
        if (($flags & self::SPLIT_TRIM) !== 0) {
            foreach ($sentences as &$sentence) {
                $sentence = $this->mbTrim($sentence);
            }
            unset($sentence);
        }
